...
Next 16 (App Router, Turbopack, Cache Components) · React 19 · TypeScript strict · Tailwind v4 · shadcn/ui · Drizzle + Neon Postgres (pooled) · Neon Auth (`@neondatabase/auth`, Managed Better Auth) · Cloudinary · next-intl · GSAP (advanced ScrollTrigger / SplitText / timeline).

**Never introduce**: Prisma, TypeORM, MUI, Redux/RTK, GraphQL, Sanity, Contentful, MDX-based CMS, Three.js / R3F, other 3D engines, pre-rendered scroll-video (Higgsfield / Monid). These are either the owner's _day-job_ tools or explicitly retired directions (see §2, §15, §16, and the 2026-08-07 pivot note in `docs/BUILD_PLAN.md`). They may appear only as _content_ (bio text), never as `package.json` dependencies.
