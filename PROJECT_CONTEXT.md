...
- **Single user only**: `devhasibulislam@gmail.com`. Neon Auth (Managed Better Auth beta, as of 2026-07-28) does not yet support disabling public sign-up at the project level — the Console shows the banner _"Anyone on the web can sign up for your app. Support for restricted signups is coming soon."_. Because of this, **the app-level defence-in-depth check is the real security boundary**: any authenticated session whose `email !== DASHBOARD_ALLOWED_EMAIL` is signed out and bounced to `/login` by both `proxy.ts` and `src/app/dashboard/layout.tsx`. When Neon adds restricted sign-up, disable public sign-up at the project level too so the app-level check becomes belt-and-braces rather than the primary gate.
