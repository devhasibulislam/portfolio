...
| De-AI-ify Chinese prose (`humanizer-zh/`)                                                                                                  | ❌ Out of scope — this project ships in `en/bn/ar/ur/he` per §10, never Chinese. Skill exists on disk but is not to be consulted. Swap for `blader/humanizer` (English) if a humanizer is actually needed. |
