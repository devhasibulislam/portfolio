...
import { bezelInner, bezelOuter } from "@/components/home/_shared";
...
/**
 * Site-wide "get in touch" block. Used at the bottom of the home page and
 * every detail page (blog post, project, experience). Copy comes from the
 * shared `home.contact` translation namespace.
 */
export async function ContactCTA({ className }: { className?: string }) {
  ...
  return (
    <section
      ...
      className={cn(
        "relative mx-auto w-full max-w-6xl px-6 py-16 sm:py-20 md:py-24",
        className,
      )}
    >
...
function ContactRow({
  ...
  // mailto: opens the mail client without a new tab.
