/**
 * Idempotent seed for the `skills` table.
 *
 * For each skill:
 *  1. Fetch its icon SVG (devicon jsDelivr for well-known logos; inline SVG
 *     literals for BullMQ / TypeORM / LangChain which aren't on devicon).
 *  2. Upload to Cloudinary under `portfolio/skills/` with a stable
 *     public_id per slug — re-runs replace the same asset instead of
 *     duplicating.
 *  3. Upsert a `media` row keyed by public_id.
 *  4. Upsert a `skills` row keyed by slug, wiring `iconMediaId`.
 *
 * Content decisions grounded in HASIBUL_ISLAM_RESUME.md and
 * PROJECT_CONTEXT §16 (senior full-stack, 7+ yrs, Node/Nest/TS core stack;
 * AI/LLM stack is production-relevant, not novelty).
 *
...
const ROWS: Row[] = [
  ...
  },
  {
    ...
    displayOrder: 150,
    status: "active",
    icon: { kind: "svg", body: TYPEORM_SVG },
  },
  {
    name: "Docker",
    slug: "docker",
  ...
  },
  {
    ...
    displayOrder: 200,
    status: "active",
    icon: { kind: "svg", body: LANGCHAIN_SVG },
  },
];
...
async function uploadIcon(row: Row): Promise<UploadResult> {
  let svg: string;
  if (row.icon.kind === "svg") {
    svg = row.icon.body;
  } else {
    // Fetch locally rather than letting Cloudinary pull the URL — some
    // CDNs (simpleicons.org, occasionally jsDelivr) reject Cloudinary's
    // fetcher with 403.
    const res = await fetch(row.icon.url, {
      headers: { "User-Agent": "portfolio-seed/1.0" },
    });
    if (!res.ok) throw new Error(`Icon fetch ${row.icon.url} → ${res.status}`);
    svg = await res.text();
  }
...
async function upsertMedia(
  ...
  const existing = await db
    .select({ id: media.id })
    .from(media)
    .where(eq(media.publicId, u.public_id))
    .limit(1);
  ...
  const [inserted] = await db.insert(media).values(values).returning();
  return inserted.id;
}
...
async function upsertSkill(
  ...
  const { icon: _icon, ...base } = row;
  void _icon;
  const existing = await db
    .select({ id: skills.id })
    .from(skills)
    .where(eq(skills.slug, row.slug))
    .limit(1);
  if (existing[0]) {
    await db
      .update(skills)
      .set({ ...base, iconMediaId, updatedAt: new Date() })
      .where(eq(skills.id, existing[0].id));
    return "updated";
  }
  await db.insert(skills).values({ ...base, iconMediaId });
  return "created";
}
...
async function main() {
  ...
  // Upload all icons first (Cloudinary is the slow leg — do it up front so
  // the pg pool isn't sitting idle between DB writes and getting dropped by
  // Neon's compute).
  console.log("Uploading icons to Cloudinary…");
  const uploaded: Array<{ row: Row; upload: UploadResult }> = [];
  for (const row of ROWS) {
    process.stdout.write(`  · ${row.name.padEnd(16)} `);
    const upload = await uploadIcon(row);
  ...
  let created = 0,
    updated = 0;
  for (const { row, upload } of uploaded) {
    const mediaId = await upsertMedia(upload, `${row.slug}.svg`);
    const outcome = await upsertSkill(row, mediaId);
    if (outcome === "created") created++;
    else updated++;
    console.log(`  · ${row.name.padEnd(16)} ${outcome}`);
  }
  console.log(`\n✓ Done. ${created} created, ${updated} updated.`);
  console.log(
    `  Revalidate the /skills page — run once from the dashboard, or restart dev server.`,
  );
}
