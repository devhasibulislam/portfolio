...
- **Single user only**: `devhasibulislam@gmail.com`. Sign-up is disabled at the Neon Auth project level. In addition, application middleware performs a defence-in-depth check: any authenticated session whose email !== the whitelisted address is signed out immediately.
...
- Sign-up, forgot-password, and email-verification flows — not built. Sign-up is disabled at the Neon Auth project level.
...
- Redux, MUI, Prisma, TypeORM, GraphQL as project dependencies (fine as *content* references only — see Section 16).
